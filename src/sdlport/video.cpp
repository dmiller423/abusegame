...
extern "C" void _exit(int)	// sdl req, not in std lib for ps4
{
	while(1) {usleep(1000);}
}
#endif
...
void set_mode(int argc, char **argv)
{
	...
	renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_SOFTWARE);	//flags.software ? SDL_RENDERER_SOFTWARE : SDL_RENDERER_ACCELERATED);
	if (renderer == NULL)
	{
...
void video_change_settings(int,bool)
{
#ifdef PS4
	SDL_SetWindowFullscreen(window, SDL_WINDOW_FULLSCREEN_DESKTOP);
#else
