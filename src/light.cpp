...
{
  volatile int cx=w;
  volatile uint8_t * di=screen_addr;
  volatile uint8_t * si=remap;
  while (cx--)
  {
                ...
                int32_t sy)
{
  ...
  volatile int dx,dy;           // x and y distances
  ...
  for (light_count=lp->total; light_count>0; light_count--)
  {
    ...
    volatile int32_t *dt=&(*lon_p)->type;
